...
def test_validate_sell():
    ...
    book_two = Book(2, "A Lua do Oriente e Outras Luas", "978-65-5580-076-0", "Christina Stephano de Queiroz", "Ilustrado, Literatura brasileira", 78.0)
    book_three = Book(3,"A Um - Poemas", "85-85851-40-6", "Robert Creeley", "Poesia, Literatura estrangeira", 50)
    book_four = Book (-1, "A Voz e o Tempo - 3a ed.", "978-65-5580-016-6", "Roberto Gambini", "Ilustrado, Psicanálise e Psicologia", 72.0) 
    book_two.stock = -777
    ...
    result_order_exits = interface_usuario.validate_sell(order_exits.id,1,book_one)
    result_verif_if_customer_exists = interface_usuario.validate_sell(order_exits.id,777,book_one)
    result_stock_valid = interface_usuario.validate_sell(order_exits.id,1,book_two.stock)
    result_verif_if_book_cost_more_zero = interface_usuario.validate_sell(order_exits.id,1,book_three)
    result_verif_id_book = interface_usuario.validate_sell(order_exits.id,1,book_four)
    ...
    book_one = Book(2, "Um livro bem ruim", "487898423", "Richarlison Pombo", "Broco mermo", 150,85)
    ...
    order_exits = Order(2, customer_pompo, datetime.today())

    result_order_exits = interface_usuario.validate_sell(order_exits.id,2,book_one)
    result_verif_if_customer_exists = interface_usuario.validate_sell(order_exits.id,666,book_one)
  
    assert result_order_exits == "Validado com sucesso!!!"
    assert result_verif_if_customer_exists == "Cliente não cadastrado"
    assert book_one.stock == 0
